...
describe('session blocks — cap enforcement', () => {
  it('rejects the 11th image (throws CapExceededError)', () => {
    for (let i = 0; i < IMAGE_HARD_CAP; i++) addImageBlock(TINY_PNG, 'gallery');
    expect(() => addImageBlock(TINY_PNG, 'gallery')).toThrow(CapExceededError);
    expect(listBlocks().filter((b) => b.kind === 'image')).toHaveLength(IMAGE_HARD_CAP);
  });

  it('rejects the 9th text block (throws CapExceededError)', () => {
    for (let i = 0; i < TEXT_HARD_CAP; i++) addTextBlock(`t${i}`, 'paste');
    expect(() => addTextBlock('overflow', 'typed')).toThrow(CapExceededError);
    expect(listBlocks().filter((b) => b.kind === 'text')).toHaveLength(TEXT_HARD_CAP);
  });

  it('CapExceededError carries the kind + cap so callers can format the message', () => {
    for (let i = 0; i < IMAGE_HARD_CAP; i++) addImageBlock(TINY_PNG, 'gallery');
    try {
      ...
      expect((e as CapExceededError).kind).toBe('image');
      expect((e as CapExceededError).cap).toBe(IMAGE_HARD_CAP);
    }
  });

  it('image cap and text cap are independent', () => {
    for (let i = 0; i < IMAGE_HARD_CAP; i++) addImageBlock(TINY_PNG, 'gallery');
    expect(() => addTextBlock('still ok', 'typed')).not.toThrow();
  });
});
