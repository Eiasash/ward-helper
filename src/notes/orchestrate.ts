...
2. O is a bedside exam in short lines. Open with "מצב כללי טוב" / alertness, then system findings (לב / ריאות / בטן / גפיים), then the one patient-specific finding the case turns on (wound + drain / fistula thrill+bruit / unilateral edema). Vitals fold into one line, and bedside point-of-care readings (fingerstick glucose) may sit in O — they are bedside findings. Serum / central-lab results (Hgb, CRP, creatinine, electrolytes, drug levels) do NOT go in O — they belong inline in the relevant *domain bullet in A.
3. A — capsule on the FIRST rehab SOAP only. On the first rehab SOAP for a patient, A opens with a one-line capsule — age + sex + key diagnosis + the acute event/date + day-of-rehab or POD. On follow-up rounds there is NO capsule — A goes straight into the problem list as delta updates (the STEPDOWN format; do not contradict it). The problem list is *domain bullets — use the "*" prefix per SOAP_STYLE (the source guide's "#" is normalized to "*"). Each bullet = domain + current status + drugs + the decision. Serum lab values and drug levels go INLINE in the relevant *domain bullet, never in O.
...
10. תפקוד: STRUCTURED subsection (per PATIENT 29/04/26 print), label: value lines in this exact order, NO prose:
