...
async function runPlaywrightFlow(scenario, browser) {
  ...
  try {
    ...
    // Find and click "register" / "the femur exemplarם" tab if it exists. Most ward-helper installs land on login first.
    const registerTab = page.getByRole('button', { name: /the femur exemplarמ|register/i }).first();
    ...
    // Click submit.
    const submitBtn = page.getByRole('button', { name: /the femur exemplarמה|הירשם|submit|create/i }).first();
