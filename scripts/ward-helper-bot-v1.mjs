 ...
 * Click the toggle button labeled "אין לי חשבון" / "the femur exemplarמה" / "register" to
...
async function switchToRegisterForm(page) {
  ...
  const togglePatterns = [
    /^the femur exemplarמה$/,                          // bare "Registration" — primary
    /אין לי חשבון/,
    /צור חשבון/,
  ...
  await sleep(800);
  for (const role of ['tab', 'button']) {
    const tab = page.getByRole(role, { name: /^the femur exemplarמה$/ }).first();
    if ((await tab.count().catch(() => 0)) > 0) {
      await tab.click({ timeout: CONFIG.actionTimeoutMs }).catch(() => {});
      await sleep(600);
      const regField = page.getByPlaceholder(/שם משתמש \(3-32 תווים/);
