...
for r in "${ROOTS[@]}"; do
  [ -d "$r" ] || continue
  # denylist — filter the קבלת הרשאות ("permissions") false positive that contains the substring the femur exemplar
  if grep -rIEn "$DENY" "$r" 2>/dev/null | grep -v 'הרשאות'; then
