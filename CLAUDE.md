...
- CSP meta in `index.html` locks `connect-src` to `self` + `toranot.netlify.app` + `*.supabase.co`. **Do not re-add `api.anthropic.com`** — direct Anthropic calls from mobile Chrome stall on multi-MB bodies.
