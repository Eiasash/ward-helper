...
## DAILY ROUNDS / SOAP — rehab and acute medicine

**Format applies to both rehab daily rounds (ביקור רופא בשיקום) and the first SOAP follow-up in acute geriatric/internal medicine wards.** Per Dr. עביד ראפת SZMC pattern. Three-pattern model based on round type.

### Section order — every SOAP, headers underlined
...
### S (subjective) — daily-asks ALWAYS, every round, every dept

Standing ask line:
- Sleep: `ישן/ה טוב בלילה / לא ישן/ה`
- Pain: `ללא כאבים / כאב X/10 / כאב מאוזן עם הטיפול הנוכחי`
- BM: `יציאות תקינות / עצירות X ימים / פעמ"ם אתמול`
- Urine: `שתן תקין / רטנציה / קטטר`
- Cooperation: `משתתף/ת בטיפול שיקומי / לא משתף/ת`

Then today's specific complaints, family requests, mood, transfer-related concerns. Pain status determines whether the patient can do today's PT — surface it in S, not buried in A.

If patient feels well: `מרגיש/ה טוב, ישן/ה טוב בלילה. ללא כאבים. שתן ויציאות תקינים. משתתף/ת בטיפול שיקומי.`

### O (objective) — bedside exam ONLY

**NEVER paste lab tables or imaging blocks here.** No `מעבדה:` block, no `הדמיה:` block.

- Vitals: `חום X.X, ד YY, ל"ד YY/YY, סטורציה NN%`
- Cooperation + cognition implicit
- 4-system: heart / lungs / abdomen / limbs
- Indication-focused additions:
    - Post-op: surgical wound (length, sutures, drainage)
    - Mobility: per-limb motor + sensation + DP
    - HD: fistula thrill+bruit, access infection signs
    - Post-CVA: cranial nerves, motor per limb segment, language
    - Pneumonia/sepsis: lung zones, mental status, perfusion
    - CHF: JVP, S3, lung crackles per zone, leg edema, weight

Labs and imaging belong in A — see below.

### A (assessment) — capsule THEN problem bullets

**FIRST follow-up after admission** opens with a 3-4 line patient capsule (Dr. עביד ראפת pattern):

```
בן/בת [age], [marital], [parent count]. מתגורר/ת [living - alone/with whom/floor/elevator/caregiver/institution]. איש קשר עיקרי: [name + relationship + phone].
רקע רפואי כולל [3-5 chronic dx prioritized for relevance].
בבסיס [pre-admission ADL/mobility one-liner: walker / wheelchair / cane / independent].
כעת לאחר [acute event] בתאריך [DD/MM]. התקבל/ה ל[ward] ביום [N] לאחר [event].

בעיות:
*[domain] - [status, relevant lab/imaging inline, decision]
```

**Subsequent follow-ups** drop the capsule. Open with one-line synthesis (`בשיקום לאחר X — מתקדם/יציב/החמרה ב-Y`), then bullets directly.

**Cross-check capsule against admission הצגת החולה.** Demographics propagate for the entire admission — wrong on day 1 = chart corruption.

### `*domain` bullet prefixes (Hebrew, asterisk — NOT #hashtag)

`*אורתופדית` `*זיהומית` `*תפקודית` `*עצמות` `*כלייתי` `*לבבי` `*נשימתי` `*נוירולוגית` `*כאב` `*עצירות` `*שינה` `*תזונתי` `*פסיכולוגית` `*המטולוגית` `*אא"ג` `*קוגניטיבי` `*לחץ דם` `*שתן` `*מטבולית` `*פצע` `*בצקת`

### Labs/imaging INLINE per problem, ONLY when actionable today

Examples:
```
*עצמות - שבר נמוך-אנרגיה, OP ידועה. ויטמין D 23.6 (חוסר), Ca 8.0 (מתחת לנורמה). יש להשלים PTH/אלבומין/Ca מתוקן/ALP. להעלות D מעל 30 לפני ביספוספונט.
...
**Rule:** if a lab/result isn't going to drive a decision today, leave it out. The acute team's discharge has the full panel — this is a daily handoff, not a chart biography.

### P (plan) — bare-verb actions with explicit timing

Timing phrases: `היום` / `מחר בבוקר` / `לפני המנה הבאה` / `יומיומית` / `במשך X ימים` / `עד תאריך DD/MM`. Usually shorter than A. STABLE patients: `המשך טיפול שיקומי` alone is acceptable.

### תוכנית טיפול (יעדי טיפול) — 1-2 lines, case header + concrete goal

Goal-by-indication table:

| Indication | Day-1 goal |
|---|---|
| Post-hip / knee replacement | `בן/בת X לאחר [procedure]. מטרה לעצמאות בניידות ובשרותים.` |
| Post-spine surgery | `מטרה לעצמאות במעברים והליכה עם עזרים.` |
| Pre-cardiac surgery | `מטרה לחיזוק תפקודי טרום-ניתוחי.` |
...
| Acute pneumonia | `מטרה לסיום אנטיביוטי, חזרה לתפקוד בסיסי, שחרור.` |
| Acute HF decompensation | `מטרה לאיזון נפח, התאמת תרופות, שחרור.` |

### Triage — which pattern?

1. **First SOAP after admission** → FIRST-DAY (capsule mandatory regardless of complexity)
2. **Subsequent + ≥2 active complexity triggers** → COMPLEX (problem-list with multiple `*domain` bullets, labs/imaging inline)
3. **Subsequent + <2 triggers** → STABLE (synthesis A, `המשך שיקום` P)

Complexity triggers: IV ABX with drug levels, post-op wound/drain/VAC, active diuresis, BP/glucose actively titrated, AKI in evolution, pending procedure, abnormal labs in evolution.

### Anti-patterns

- ❌ Capsule in S instead of A
- ❌ Lab tables or imaging blocks in O
- ❌ Skipping daily-asks because patient "looked fine"
- ❌ Repeating capsule on subsequent rounds
- ❌ `#hashtag` instead of `*domain` bullets
- ❌ Labs en masse in A without tying each to a problem and decision
- ❌ `מוקדם מדי לקבוע מטרה` past day 7 (or day 0 for non-CVA indications)
